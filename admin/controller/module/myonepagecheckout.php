<?php
class ControllerModuleMyonepagecheckout extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('module/myonepagecheckout');

		$this->document->setTitle(strip_tags($this->language->get('heading_title')));

		$this->load->model('extension/module');

		$this->load->model('module/myonepagecheckout');
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_module_myonepagecheckout->editSettingMyonepagecheckout('myonepagecheckout', $this->request->post);
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/myonepagecheckout', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/myonepagecheckout', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/myonepagecheckout', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/myonepagecheckout', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['config_show_newstorecheckout'])) {
			$data['config_show_newstorecheckout'] = $this->request->post['config_show_newstorecheckout'];
		} else {
			$data['config_show_newstorecheckout'] = $this->config->get('config_show_newstorecheckout');
		}
		...
		$this->response->setOutput($this->load->view('module/myonepagecheckout.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/myonepagecheckout')) {
