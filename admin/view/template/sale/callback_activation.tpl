...
var license_key = $('#license_key').val();
	$.post('index.php?route=sale/callback/activation&token=<?php echo $token; ?>', 'license_key=' + license_key);
	}
});
</script>
<style>
			...
			$('#submitChangeStatus').bind('click',function() {
				...
				var success = 'false';
					$.ajax({
						...
						url:'index.php?route=sale/callback/deactivation&token=<?php echo $token; ?>',			
