...
class NewstoreMinifier {
    ...
    private function printStyle($href) {
        return '<link rel="stylesheet" href="' . NewstoreUtils::staticAsset($this->addNewstorelVersion($href, $this->minify_css)) . '"/>' . PHP_EOL;
    }

    private function printScript($src, $def = '') {
        $def = rtrim($def) . ' ';
        return '<script type="text/javascript"' . $def . 'src="' . NewstoreUtils::staticAsset($this->addNewstorelVersion($src, $this->minify_css)) .  '"></script>' . PHP_EOL;
    }
