<?php
class ControllerCommonHomeshemeNewstorePos21 extends Controller {
	public function index() {
		...
		$data['modules'] = array();		
		$modules = $this->model_design_layout->getLayoutModules($layout_id, 'position_21');
		...
		foreach ($modules_newstore as $module) {
			if($module['position'] == 'position_21') {
				$part = explode('.', $module['code']);
				if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
					$data['modules'][] = $this->load->controller('module/' . $part[0]);
				}
		...
		return $this->load->view($this->config->get('config_template') . '/template/common/homesheme/newstore_pos21.tpl', $data);
