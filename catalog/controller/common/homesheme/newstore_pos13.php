<?php
class ControllerCommonHomeshemeNewstorePos13 extends Controller {
	public function index() {
		...
		$data['modules'] = array();		
		$modules = $this->model_design_layout->getLayoutModules($layout_id, 'position_13');
		...
		foreach ($modules_newstore as $module) {
			if($module['position'] == 'position_13') {
				$part = explode('.', $module['code']);
				if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
					$data['modules'][] = $this->load->controller('module/' . $part[0]);
				}
		...
		return $this->load->view($this->config->get('config_template') . '/template/common/homesheme/newstore_pos13.tpl', $data);
