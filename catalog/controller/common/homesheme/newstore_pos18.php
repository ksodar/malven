<?php
class ControllerCommonHomeshemeNewstorePos18 extends Controller {
	public function index() {
		...
		$data['modules'] = array();		
		$modules = $this->model_design_layout->getLayoutModules($layout_id, 'position_18');
		...
		foreach ($modules_newstore as $module) {
			if($module['position'] == 'position_18') {
				$part = explode('.', $module['code']);
				if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
					$data['modules'][] = $this->load->controller('module/' . $part[0]);
				}
		...
		return $this->load->view($this->config->get('config_template') . '/template/common/homesheme/newstore_pos18.tpl', $data);
