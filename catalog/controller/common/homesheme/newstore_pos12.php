<?php
class ControllerCommonHomeshemeNewstorePos12 extends Controller {
	public function index() {
		...
		$data['modules'] = array();		
		$modules = $this->model_design_layout->getLayoutModules($layout_id, 'position_12');
		...
		foreach ($modules_newstore as $module) {
			if($module['position'] == 'position_12') {
				$part = explode('.', $module['code']);
				if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
					$data['modules'][] = $this->load->controller('module/' . $part[0]);
				}
		...
		return $this->load->view($this->config->get('config_template') . '/template/common/homesheme/newstore_pos12.tpl', $data);
