...
class ControllerCheckoutNewstoreCheckout extends Controller { 
	...
	public function index() { 
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/newstorecheckout.tpl')) {
			$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/newstorecheckout.tpl';
		} else {
			$this->template  = $template = 'default/template/checkout/newstorecheckout.tpl';
		}
	...
	public function login_validate($data = array()) {
		...
		$json = array();		
		if ($this->customer->isLogged()) {
			$json['redirect'] = $this->url->link('checkout/newstorecheckout', '', 'SSL');			
		}		
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
...
//			$json['redirect'] = $this->url->link('checkout/cart');
	...
	public function shipping_address_validate(&$data = array()) {
		...
		if (!$json) {
			...
			if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new') { 
				...
				$custom_fields = $this->model_account_custom_field->getCustomFields(array('filter_customer_group_id' => $this->config->get('config_customer_group_id')));
				foreach ($custom_fields as $shipping_custom_field) {
					if (($shipping_custom_field['location'] == 'address') && $shipping_custom_field['required'] && empty($this->request->post['shipping_custom_field'][$shipping_custom_field['custom_field_id']])) {
						$json['error']['shipping_custom_field[' . $shipping_custom_field['custom_field_id'].']'] = sprintf($this->language->get('error_custom_field'), $shipping_custom_field['name']);
					}
				}
				if (!$json) {						
					// Default Shipping Address
	...
	public function shipping_method($render = true, &$data = array()) {
		...
		if ($render !== false){
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/newstoreshipping_method.tpl')) {
				$template = $this->template = $this->config->get('config_template') . '/template/checkout/newstoreshipping_method.tpl';
			} else {
				$template = $this->template = 'default/template/checkout/newstoreshipping_method.tpl';
			}
				$this->response->setOutput($this->load->view($template, $data));			 
		}
  	}	
	...
	public function payment_address($render = true, &$data  = array()) {
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($render !== false){			
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/newstorepayment_method.tpl')) 
			{
				$template =  $this->template = $this->config->get('config_template') . '/template/checkout/newstorepayment_method.tpl';
			} else {
				$template =  $this->template = 'default/template/checkout/newstorepayment_method.tpl';
			}			
			$this->response->setOutput($this->load->view($template, $data));
		}
  	}
	...
	public function cart($render = true, &$data = array()){
		...
		if (!isset($this->session->data['vouchers'])) {
      	...
      	$data['breadcrumbs'][] = array(
            ...
            foreach ($products as $product) {
               ...
               if(isset($product['cart_id'])){
                ...
                $data['products'][] = array(
                    ...
                    'remove'              => $this->url->link('checkout/cart', 'remove=' . $card_id),
			...
			$data['checkout'] = $this->url->link('checkout/newstorecheckout', '', 'SSL');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/newstorecart.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/checkout/newstorecart.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/checkout/newstorecart.tpl', $data));
				}
	...
	public function confirm($render = true, &$data = array()) {
		...
		if ($this->cart->hasShipping()) {
			...
			if (!isset($this->session->data['shipping_address'])) {
				$redirect = $this->url->link('checkout/newstorecheckout', '', 'SSL');
			}
