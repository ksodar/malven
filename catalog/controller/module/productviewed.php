...
class ControllerModuleProductviewed extends Controller {
	public function index($setting) {
		...
		if (!empty($this->session->data['productviewed']) && count($this->session->data['productviewed']) > 0) {
	
			$result = array_slice($this->session->data['productviewed'], 0, $setting['limit']);
		...
		if ($data['products']) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/productviewed.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/productviewed.tpl', $data);
			} else {
				return $this->load->view('default/template/module/productviewed.tpl', $data);
			}
		}
	}
}
