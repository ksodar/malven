...
class ControllerModuleAlltabs extends Controller {
	public function index($setting) {
		...
		foreach ($products as $product_id){
			...
			if ($product_info) {
				...
				if ($data['show_options']==1) {
					...
					$data['featured_products'][] = array(
						...
						'str_timer_3'	  	=> $str_timer_3,
						'options'	  		=> $options,
						'minimum'     		=> ($product_info['minimum'] > 0) ? $result['minimum'] : 1,
						'price_no_format' 	=> $price_no_format,
						'special_no_format' => $special_no_format,	
						'product_quantity' 	=> $product_quantity,
						/* MICRODATA */
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/alltabs.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/alltabs.tpl', $data);
			} else {
				return $this->load->view('default/template/module/alltabs.tpl', $data);
			}
