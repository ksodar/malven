...
class ControllerModuleAutosearch extends Controller { 
	...
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/autosearch.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/module/autosearch.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/module/autosearch.tpl', $data));
		}			
  	}
	public function ajaxLiveSearch() {
					...
					if(!empty($this->request->get['filter_name'])){
						...
						$this->load->model('module/autosearch');
						...
						$results = (array) $this->model_module_autosearch->ajaxLiveSearch($filterdata);
