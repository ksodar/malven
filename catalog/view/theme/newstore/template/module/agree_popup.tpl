	...
	<div class="agree-footer">
		<div class="text-right">
			...
			<?php if($agreedata['status_btn_agree']){ ?>
				<button <?php if($agreedata['method_agree_popup']=='method_session') { ?>onclick="saveSessionAgree();"<?php } else { ?>onclick="saveCookieAgree();"<?php } ?> class="btn-agree-disagree btn-agree" type="button"><?php if($agreedata['icon_btn_agree'] !='') { ?><i class="<?php echo $agreedata['icon_btn_agree']?> fa-fw"></i><?php } ?><?php echo isset($agreedata['btn_agree'][$lang_id]) ? $agreedata['btn_agree'][$lang_id]['text'] : ''; ?></button>
			<?php } ?>
		</div>
	</div>
	<?php } ?>
...
function saveCookieAgree() {   
	$.ajax({
        type: 'get',
        url:  'index.php?route=module/agree_popup/saveAgreeCookie&saveAgreeCookie=<?php echo isset($agreedata['cookie_day']) ? $agreedata['cookie_day'] : '1'; ?>'
    });
	$('.popup_agree').popup('hide');
}
<?php } ?>
