...
<?php $gen_carousel_latest = rand(10000, 50000);?>
<div class="container-module">
<div class="title-module"><span><?php echo $heading_title_latest; ?></span></div>
<div class="product-slider">
<div class="container-modules latest carousel_numb_latest<?php echo $gen_carousel_latest;?> owl-carousel">
   ...
   <div class="item">
    <div itemtype="http://schema.org/Product" itemprop="itemListElement" itemscope class="product-thumb transition">
      ...
      <div class="image">	
		...
		<?php if($ns_on_off_latest_slider_additional_image =='1') { ?>		
		...
		<?php } elseif($ns_on_off_latest_slider_additional_image =='2') { ?>
		...
		<?php if($show_special_timer_module =='1' && $product['special']){ ?>
			<div class="action-timer">
				<div id="countdown-latest-<?php echo $gen_carousel_latest;?>-<?php echo $product['product_id'];?>"></div>		
			</div>
...
$(function(){
	...
	$('#countdown-latest-<?php echo $gen_carousel_latest;?>-<?php echo $product['product_id'];?>').countdown({
		timestamp	: ts,
	  ...
	  <div class="caption">
	   <div class="actions-quick-order">
		 ...
		 <?php if($ns_on_off_module_quick_order =='1') { ?>
			<?php if($ns_on_off_latest_fastorder !='0') { ?>
		...
		<?php if($ns_on_off_latest_model_product !='0') { ?>
			<div class="product-model"><?php echo $product['model']?></div>
		<?php } ?>
		<?php if($ns_on_off_latest_description !='0') { ?>
        <div itemprop="description" class="product-description"><?php echo $product['description']; ?></div>
		<?php } ?>
		<?php if ($show_options) { ?>
			<div id="option_latest_<?php echo $product['product_id']; ?>" class="option latest-opt">
				...
				<?php if ($option['type'] == 'select'){ ?>
					<div class="form-group">
						...
						<div id="input-option-special<?php echo $option['product_option_id']; ?>">
							<select onchange="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');" name="option[<?php echo $option['product_option_id']; ?>]" id="input-value-<?php echo $option['product_option_id']; ?>_special" class="form-control">
            ...
            <div class="form-group">
              ...
              <div id="input-option-latest<?php echo $option['product_option_id']; ?>">
                ...
                <div class="radio-checbox-options">
					<input data-option-prefix="<?php echo $option_value['price_prefix']; ?>" data-option-price="<?php echo $option_value['price_value']; ?>" onchange="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="input-value-<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>_featured" />
            ...
            <div class="form-group">
              ...
              <div id="input-option-latest<?php echo $option['product_option_id']; ?>">
                <?php foreach ($option['product_option_value'] as $option_value) { ?>
                <div class="radio-checbox-options">
                  <input data-option-prefix="<?php echo $option_value['price_prefix']; ?>" data-option-price="<?php echo $option_value['price_value']; ?>" onchange="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');" type="checkbox" name="option[<?php echo $option['product_option_id']; ?>][]" value="<?php echo $option_value['product_option_value_id']; ?>" id="input-value-<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>_featured" />
					<label for="input-value-<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>_featured">
                    <span class="option-name"><?php echo $option_value['name']; ?></span>
						<?php if ($option_value['price']) { ?>
						<span class="option-price"><?php echo $option_value['price_prefix']; ?><?php echo $option_value['price']; ?></span>
		...
		<?php } ?>
		<?php if($ns_on_off_latest_rating !='0') { ?>
		<div class="rating">
			...
			<?php if($ns_on_off_latest_quantity_reviews !='0') { ?> 
			...
			<div class="quantity_cont">
				<div class="input-group">
					<span class="input-group-btn">
						<button class="btn btn-quantity-minus" onclick="btnminus_cat_price_latest<?php echo $product['product_id'];?>('<?php echo $product['minimum']; ?>');" type="button">-</button>
					</span>
						<input id="input_quantity_mod_latest<?php echo $product['product_id'];?>" class="form-control input-number-quantity<?php echo $product['product_id'];?>" name="quantity" onkeyup="validate_quantity(this,'<?php echo $product['minimum']; ?>')" oninput="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>, <?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');" size="2" value="<?php echo $product['minimum']; ?>">
					<span class="input-group-btn">
						<button class="btn btn-quantity-plus" onclick="btnplus_cat_price_latest<?php echo $product['product_id'];?>();" type="button" >+</button>
					</span>
				</div>	
			</div>
		</div>
		<script type="text/javascript">
				function btnminus_cat_price_latest<?php echo $product['product_id'];?>(a){
					document.getElementById("input_quantity_mod_latest<?php echo $product['product_id'];?>").value>a?document.getElementById("input_quantity_mod_latest<?php echo $product['product_id'];?>").value--:document.getElementById("input_quantity_mod_latest<?php echo $product['product_id'];?>").value=a;										
					recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');						
				}
				function btnplus_cat_price_latest<?php echo $product['product_id'];?>(){
					document.getElementById("input_quantity_mod_latest<?php echo $product['product_id'];?>").value++;
					recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>','latest');					
				};																				
		</script>
		<?php } ?>
		<div class="actions">
			<?php if($ns_on_off_latest_wishlist !='0') { ?>
			...
			<?php } else { ?>
				<div class="cart"><button class="btn btn-general" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>','latest', get_cart_quantity('<?php echo $product['product_id']; ?>','.carousel_numb_latest<?php echo $gen_carousel_latest;?>'));"><?php if($change_text_cart_button_out_of_stock ==1 && $product['product_quantity'] <= 0) { ?><span><?php echo $disable_cart_button_text; ?></span><?php } else { ?><i class="fa fa-shopping-basket"></i> <span><?php echo $button_cart; ?></span><?php } ?></button></div>
			<?php } ?>
			<?php if($ns_on_off_latest_compare !='0') { ?>
...
<script type="text/javascript">
$('.carousel_numb_latest<?php echo $gen_carousel_latest;?>').each(function() {
if ($(".carousel_numb_latest<?php echo $gen_carousel_latest;?>").parents("#column-left, #column-right, .position-no-owl").length) {
...
$('.carousel_numb_latest<?php echo $gen_carousel_latest;?>').owlCarousel({
	responsiveBaseWidth: ".carousel_numb_latest<?php echo $gen_carousel_latest;?>",
...
$(".carousel_numb_latest<?php echo $gen_carousel_latest;?> .additional-image").removeClass('hidden');	
	$(".carousel_numb_latest<?php echo $gen_carousel_latest;?> .image-carousel").owlCarousel({
				navigation : true, 
				slideSpeed : 300,
				pagination:false,
				...
				paginationSpeed : 400,
				singleItem:true,
				transitionStyle: '<?php echo $ns_select_latest_additional_animate_method;?>'
	});
});
	<?php } ?>
}
