<div id="popup-editor-product">
	<div class="title-editor"><?php echo $text_option;?></div>
	<form id="option-form" action="POST" method="post" enctype="multipart/form-data" class="form-horizontal">
                  ...
                  <div class="tab-content">
                    ...
                    <div class="tab-pane" id="tab-option<?php echo $option_row; ?>">
                      ...
                      <div class="table-responsive">
                        <table id="option-value<?php echo $option_row; ?>" class="table table-striped table-bordered table-hover" style="background:#fff;">
                          <thead>
                            <tr>
                          ...
                          <tbody>
                            ...
                            <tr id="option-value-row<?php echo $option_value_row; ?>">
                              ...
                              <td class="text-left"><button type="button" onclick="$(this).tooltip('destroy');$('#option-value-row<?php echo $option_value_row; ?>').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-delete-editor"><i class="fa fa-minus-circle"></i></button></td>
                            </tr>
                          ...
                          <tfoot>
                            <tr>
                              <td colspan="6"></td>
                              <td class="text-left"><button type="button" onclick="addOptionValue('<?php echo $option_row; ?>');" data-toggle="tooltip" title="<?php echo $button_add_option_value; ?>" class="btn btn-add-editor"><i class="fa fa-plus-circle"></i></button></td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
	...
	<div class="row mtb10">
		<div class="col-xs-12 text-center">
			<button class="btn-editor btn-primary-editor" type="button" data-toggle="collapse" data-target="#change_edit_options" aria-expanded="false" aria-controls="collapseExample">
			<?php echo $button_last_change;?>
			</button>
		</div>
		</div>
		<div class="row collapse" id="change_edit_options">
		<div class="col-xs-12">
		<table class="table table-bordered table-hover" style="background:#fff;">
		<thead>
		  <tr>
			<th class="text-center"><?php echo $ch_user; ?></th>
			<th class="text-center"><?php echo $ch_option; ?></th>
			<th class="text-center"><?php echo $ch_date; ?></th>
		  </tr>
		</thead>
		<tbody>
		<?php $i=0;?>
			<?php foreach ($change_edit_options as $results) { ?>
				<?php if($i >= 10) break;?>
				<tr>
					<td class="text-center"><div class="ch-user-name"><?php echo $results['user_name'];?></div></label></td>
					<td class="text-center"><?php if($results['options'] !='') { ?>
	...
	<div class="text-right">
		...
		<button onclick="saveEditorOption();" class="btn-editor btn-save-editor" type="button"><?php echo $button_save;?></button>
	</div>

<script type="text/javascript">
function saveEditorOption() {
	$.ajax({
			url: 'index.php?route=module/editproduct/editoption/saveEditorOption&product_id=<?php echo $product_id;?>',
			type: 'post',
			dataType: 'json',
			data: $('#option-form').serialize(),
			beforeSend: function() {
				$('.popup-alert').remove();
				$('#option-form').prepend('<div id="loading_img_svg"></div>');
				$('#option-form').css('opacity',0.5);
				loading_img_svg(true);
			},
			complete: function() {
				loading_img_svg(false);
				$('#option-form').css('opacity',1);
			},
			success: function(json) {				
				$('.popup-alert').remove();
				if (json['error']) {
					$('#option-form').before('<div class="popup-alert alert alert-danger"><i class="fa fa-info-circle"></i> '+ json['error'] +' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				}
				if (json['success']) {
					$('#option-form').before('<div class="popup-alert alert alert-success"><i class="fa fa-info-circle"></i> '+ json['success'] +' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				location.reload();
				}
			},
...
<script src="catalog/view/editproduct/js/datetimepicker/locale/<?php echo $langpicker;?>.js" type="text/javascript"></script>
...
$('input[name=\'option\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=module/editproduct/editoption/autocomplete_option&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',			
			success: function(json) {
