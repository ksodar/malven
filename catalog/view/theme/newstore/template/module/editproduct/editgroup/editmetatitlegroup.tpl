<div id="popup-editor-product">
	<div class="title-editor"><?php echo $entry_meta_title;?></div>
		...
		<div class="tab-content">
            <div class="tab-pane active" id="seo-general-tab">
			  <form class="form-horizontal" id="form-product-meta-title" enctype="multipart/form-data" method="post">
				...
				<div class="form-group">
					<div class="col-sm-12" id="title_load_select">
						...
						<div class="input-group"><span class="input-group-addon"><?php if($VERSION < 2.2){?><img src="admin/view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /><?php } else { ?><img src="admin/language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /><?php } ?></span>
							 <input type="text" name="product_description[<?php echo $language['language_id']; ?>][meta_title]" value="<?php if(!empty($sel_seo_tempalte)){ ?><?php echo isset($sel_seo_tempalte[$language['language_id']]) ? $sel_seo_tempalte[$language['language_id']]['meta_title'] : ''; ?><?php } else { ?><?php echo isset($seo_prod[$language['language_id']]) ? $seo_prod[$language['language_id']]['seo_prod_meta_title'] : ''; ?><?php } ?>" class="form-control" />
						</div>
					 <?php } ?>
					</div>
				</div>
...
$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoTitleLoad&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');

function saveSeoTitleNew(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoTitleNew&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		...
		$('#popup-editor-product .alert').remove();
			...
			if (json['success']) {
				...
				$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoTitleLoad&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');
				$('#title_load_select').load('index.php?route=module/editproduct/editgroup/quickedit_metaTitleGroup&seo_category_id='+ <?php echo $seo_category_id;?> +'&token=<?php echo $token; ?> #title_load_select > div');				
				setTimeout(function(){
					$('.tooltip').remove();	
					$('#popup-editor-product .alert').remove();									
				}, 1000)
...
function applySeoTitle(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/applySeoTitle&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-title').serialize(),
		success: function(json) {
		$('#popup-editor-product .alert').remove();
			if (json['warning']) {
				$('#form-product-list-seo').before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i> ' + json['warning'] + '</div>');	
...
function deleteSeoTitle(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/deleteSeoTitle&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
...
$('#button-save-seo').on('shown.bs.popover', function() {
	$('#button-create-seo').on('click', function() {	
		$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoTemplateTitle&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-title').serialize(),
			beforeSend: function() {
				$('#popup-editor-product .btn-save-editor').button('loading');
				$('#form-product-meta-title').prepend('<div id="loading_img_svg"></div>');
				$('#form-product-meta-title').css('opacity',0.5);
				loading_img_svg(true);
			},
			complete: function() {
				loading_img_svg(false);
				$('#popup-editor-product .btn-save-editor').button('reset');
				$('#form-product-meta-title').css('opacity',1);
			},
			success: function(json) {
			$('#popup-editor-product .alert').remove();
				if (json['warning']) {
					$('#form-product-meta-title').before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i> ' + json['warning'] + '</div>');	
				}
				if (json['success']) {
					$('#form-product-meta-title').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '</div>');			
					$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoTitleLoad&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');
					$('.popover').remove();	
					setTimeout(function(){
						$('.tooltip').remove();	
...
function saveGroupEditor() {	
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/productGroupMetaTitleChange&token=<?php echo $token;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-title').serialize(),
		beforeSend: function() {
