<div id="popup-editor-product">
	<div class="title-editor"><?php echo $entry_meta_h1;?></div>
		...
		<div class="tab-content">
            <div class="tab-pane active" id="seo-general-tab">
			  <form class="form-horizontal" id="form-product-meta-h1" enctype="multipart/form-data" method="post">
				...
				<div class="form-group">
					<div class="col-sm-12" id="h1_load_select">
						...
						<div class="input-group"><span class="input-group-addon"><?php if($VERSION < 2.2){?><img src="admin/view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /><?php } else { ?><img src="admin/language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /><?php } ?></span>
							 <input type="text" name="product_description[<?php echo $language['language_id']; ?>][meta_h1]" value="<?php if(!empty($sel_seo_tempalte)){ ?><?php echo isset($sel_seo_tempalte[$language['language_id']]) ? $sel_seo_tempalte[$language['language_id']]['meta_h1'] : ''; ?><?php } else { ?><?php echo isset($seo_prod[$language['language_id']]) ? $seo_prod[$language['language_id']]['seo_prod_meta_h1'] : ''; ?><?php } ?>" class="form-control" />
						</div>
					 <?php } ?>
					</div>
				</div>
...
$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoH1Load&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');

function saveSeoH1New(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoH1New&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		...
		$('#popup-editor-product .alert').remove();
			...
			if (json['success']) {
				...
				$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoH1Load&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');
				$('#h1_load_select').load('index.php?route=module/editproduct/editgroup/quickedit_metaH1Group&seo_category_id='+ <?php echo $seo_category_id;?> +'&token=<?php echo $token; ?> #h1_load_select > div');				
				setTimeout(function(){
					$('.tooltip').remove();	
					$('#popup-editor-product .alert').remove();									
				}, 1000)
...
function applySeoH1(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/applySeoH1&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-h1').serialize(),
		success: function(json) {
		$('#popup-editor-product .alert').remove();
			if (json['warning']) {
				$('#form-product-list-seo').before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i> ' + json['warning'] + '</div>');	
...
function deleteSeoH1(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/deleteSeoH1&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-h1').serialize(),
		success: function(json) {
...
$('#button-save-seo').on('shown.bs.popover', function() {
	$('#button-create-seo').on('click', function() {	
		$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoTemplateH1&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-h1').serialize(),
			beforeSend: function() {
				$('#popup-editor-product .btn-save-editor').button('loading');
				$('#form-product-meta-h1').prepend('<div id="loading_img_svg"></div>');
				$('#form-product-meta-h1').css('opacity',0.5);
				loading_img_svg(true);
			},
			complete: function() {
				loading_img_svg(false);
				$('#popup-editor-product .btn-save-editor').button('reset');
				$('#form-product-meta-h1').css('opacity',1);
			},
			success: function(json) {
			$('#popup-editor-product .alert').remove();
				if (json['warning']) {
					$('#form-product-meta-h1').before('<div class="alert alert-danger"><i class="fa fa-check-circle"></i> ' + json['warning'] + '</div>');	
				}
				if (json['success']) {
					$('#form-product-meta-h1').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '</div>');			
					$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoH1Load&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');
					$('.popover').remove();	
				}
			}
...
function saveGroupEditor() {	
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/productGroupMetaH1Change&token=<?php echo $token;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-h1').serialize(),
		beforeSend: function() {
