<div id="popup-editor-product">
	<div class="title-editor"><?php echo $entry_meta_tag;?></div>
				...
				<div class="form-group">
					<div class="col-sm-12" id="tag_load_select">
						...
						<div class="input-group"><span class="input-group-addon"><?php if($VERSION < 2.2){?><img src="admin/view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /><?php } else { ?><img src="admin/language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /><?php } ?></span>
							 <input type="text" name="product_description[<?php echo $language['language_id']; ?>][tag]" value="<?php if(!empty($sel_seo_tempalte)){ ?><?php echo isset($sel_seo_tempalte[$language['language_id']]) ? $sel_seo_tempalte[$language['language_id']]['tag'] : ''; ?><?php } else { ?><?php echo isset($seo_prod[$language['language_id']]) ? $seo_prod[$language['language_id']]['seo_prod_meta_tag'] : ''; ?><?php } ?>" class="form-control" />
						</div>
					 <?php } ?>
					</div>
				</div>
...
$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoTagLoad&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');

function saveSeoTagNew(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoTagNew&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
		...
		$('#popup-editor-product .alert').remove();
			...
			if (json['success']) {
				...
				$('#list_seo').load('index.php?route=module/editproduct/editgroup/seoTagLoad&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>');
				$('#tag_load_select').load('index.php?route=module/editproduct/editgroup/quickedit_metaTagGroup&seo_category_id='+ <?php echo $seo_category_id;?> +'&token=<?php echo $token; ?> #tag_load_select > div');				
				setTimeout(function(){
					$('.tooltip').remove();	
					$('#popup-editor-product .alert').remove();									
				}, 1000)
...
function applySeoTag(seo_id) {
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/applySeoTag&token=<?php echo $token;?>&seo_id='+ seo_id +'&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
...
$('#button-save-seo').on('shown.bs.popover', function() {
	$('#button-create-seo').on('click', function() {	
		$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/saveSeoTemplateTag&token=<?php echo $token;?>&seo_category_id=<?php echo $seo_category_id;?>',
		type: 'post',
		dataType: 'json',
...
function saveGroupEditor() {	
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/productGroupMetaTagChange&token=<?php echo $token;?>',
		type: 'post',
		dataType: 'json',
		data: $('.editor-group-checked-product input.group-editor-checkbox[type=\'checkbox\']:checked, #form-product-meta-tag').serialize(),
		beforeSend: function() {
