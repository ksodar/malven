<div id="popup-editor-product">
	<div class="title-editor"><?php echo $entry_filter;?></div>
	  <form class="form-horizontal" id="form-product-filter" enctype="multipart/form-data" method="post">
			<div class="form-group">
				<div class="col-sm-12">
					<div class="alert alert-info"><i class="fa fa-info-circle"></i> <?php echo $text_info_filter;?></div>
				</div>
			</div>
			<div class="form-group">
                <label class="col-sm-3 desc_edit_label" for="input-filter"><?php echo $entry_filter; ?></label>
                <div class="col-sm-9">
...
function saveGroupEditor() {	
	$.ajax({
		url: 'index.php?route=module/editproduct/editgroup/changeProductFilterGroup&token=<?php echo $token;?>',
		type: 'post',
		dataType: 'json',
...
$('#form-product-filter input[name=\'filter\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=module/editproduct/editgroup/autocompleteFilter&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
