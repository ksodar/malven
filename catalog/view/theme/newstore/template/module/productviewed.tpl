...
<?php $gen_carousel_product_viewed = rand(100, 500);?>
<div class="container-module">
	<div class="title-module"><span><?php echo $heading_title; ?></span></div>
	<div class="product-slider">
		<div class="container-modules featured carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?> owl-carousel">
					...
					<div class="item">
						...
						<?php } ?>
							...
							<div class="stickers-ns">
								...
								<?php if($ns_on_off_featured_slider_additional_image =='1') { ?>		
									...
									<?php foreach ($product['additional_img'] as $img) { ?>			
										<a class="additional-image hidden" href="<?php echo $product['href']; ?>">
										<img src="<?php echo $img;?>" alt="<?php echo $product['name']; ?>" title="<?php echo $product['name']; ?>" class="img-responsive"/>
										</a>			
									<?php } ?>
									</div>
								<?php } elseif($ns_on_off_featured_slider_additional_image =='2') { ?>
								<div class="image-carousel">
							...
							<?php if($show_special_timer_module =='1' && $product['special']){ ?>
								<div class="action-timer">
									<div id="countdown-viewed-<?php echo $gen_carousel_product_viewed;?>-<?php echo $product['product_id'];?>"></div>		
								</div>
							...
							$(function(){
								...
								$('#countdown-viewed-<?php echo $gen_carousel_product_viewed;?>-<?php echo $product['product_id'];?>').countdown({
									timestamp	: ts,
							...
							<?php } ?>								
								...
								<div class="caption">
									<div class="actions-quick-order">
										<div class="quick-order">
										  <?php if($ns_on_off_module_quick_order =='1') { ?>
											  <?php if($ns_on_off_featured_fastorder !='0') { ?>
									...
									<?php if($ns_on_off_featured_model_product !='0') { ?>
										<div class="product-model"><?php echo $product['model']?></div>
									<?php } ?>
									<?php if($ns_on_off_featured_description !='0') { ?>
										<div itemprop="description" class="product-description"><?php echo $product['description']; ?></div>
									<?php } ?>
									<?php if ($show_options) { ?>
									<div id="option_productviewed<?php echo $gen_carousel_product_viewed;?>_<?php echo $product['product_id']; ?>" class="option productviewed-opt">
										<?php foreach ($product['options'] as $key => $option) { ?>
											<?php if ($option['type'] == 'select'){ ?>
												<div class="form-group">
													...
													<div id="input-option-productviewed<?php echo $option['product_option_id']; ?>">
														<select onchange="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>','productviewed<?php echo $gen_carousel_product_viewed;?>');" name="option[<?php echo $option['product_option_id']; ?>]" id="input-value-<?php echo $option['product_option_id']; ?>_productviewed" class="form-control">
									...
									<div class="form-group">
									  ...
									  <div id="input-option-productviewed<?php echo $option['product_option_id']; ?>">
										...
										<div class="radio-checbox-options">
											...
											<label for="input-value-<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>_productviewed">
								...
								<?php } ?>
									<?php if($ns_on_off_featured_rating !='0') { ?>
										<div class="rating">
										...
										<div class="quantity_cont">
											<div class="input-group">
												<span class="input-group-btn">
													<button class="btn btn-quantity-minus" onclick="btnminus_cat_price_productviewed<?php echo $product['product_id'];?>('<?php echo $product['minimum']; ?>');" type="button">-</button>
												</span>
													<input id="input_quantity_mod_productviewed<?php echo $product['product_id'];?>" class="form-control input-number-quantity<?php echo $product['product_id'];?>" name="quantity" onkeyup="validate_quantity(this,'<?php echo $product['minimum']; ?>')" oninput="recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>, <?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>','productviewed<?php echo $gen_carousel_product_viewed;?>');" size="2" value="<?php echo $product['minimum']; ?>">
												<span class="input-group-btn">
													<button class="btn btn-quantity-plus" onclick="btnplus_cat_price_productviewed<?php echo $product['product_id'];?>();" type="button" >+</button>
												</span>
											</div>	
										</div>
									</div>
									<script type="text/javascript">
											function btnminus_cat_price_productviewed<?php echo $product['product_id'];?>(a){
												document.getElementById("input_quantity_mod_productviewed<?php echo $product['product_id'];?>").value>a?document.getElementById("input_quantity_mod_productviewed<?php echo $product['product_id'];?>").value--:document.getElementById("input_quantity_mod_productviewed<?php echo $product['product_id'];?>").value=a;										
												recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>','productviewed<?php echo $gen_carousel_product_viewed;?>');						
											}
											function btnplus_cat_price_productviewed<?php echo $product['product_id'];?>(){
												document.getElementById("input_quantity_mod_productviewed<?php echo $product['product_id'];?>").value++;
												recalc_quantity(<?php echo $product['product_id'];?>,<?php echo $product['minimum']; ?>,<?php echo $product['price_no_format'];?>,'<?php echo $product['special_no_format'];?>','.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>','productviewed<?php echo $gen_carousel_product_viewed;?>');					
											};																				
									</script>
									<?php } ?>
									<div class="actions">
										...
										<?php } else { ?>
											<div class="cart"><button class="btn btn-general" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>','productviewed<?php echo $gen_carousel_product_viewed;?>', get_cart_quantity('<?php echo $product['product_id']; ?>','.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>'));"><?php if($change_text_cart_button_out_of_stock ==1 && $product['product_quantity'] <= 0) { ?><span><?php echo $disable_cart_button_text; ?></span><?php } else { ?><i class="fa fa-shopping-basket"></i> <span><?php echo $button_cart; ?></span><?php } ?></button></div>
										<?php } ?>
...
<script type="text/javascript">
$('.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>').each(function() {
if ($(".carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>").parents("#column-left, #column-right, .position-no-owl").length) {
...
$('.carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>').owlCarousel({
	responsiveBaseWidth: ".carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>",
...
if (!$(".carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?>").parents("#column-left, #column-right, .position-no-owl").length) {
<?php if($ns_on_off_featured_slider_additional_image !='0') { ?>
$(window).load(function() {
$(".carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?> .additional-image").removeClass('hidden');
	$(".carousel_numb_product_viewed<?php echo $gen_carousel_product_viewed;?> .image-carousel").owlCarousel({
				navigation : true, 
				pagination:false,
				navigationText: ['<div class="btn btn-carousel-image-additional next-prod"><i class="fa fa-angle-left arrow"></i></div>', '<div class="btn btn-carousel-image-additional prev-prod"><i class="fa fa-angle-right arrow"></i></div>'],		
				singleItem:true,
				transitionStyle: '<?php echo $ns_select_featured_additional_animate_method;?>'
	});
});	
<?php } ?>
}
